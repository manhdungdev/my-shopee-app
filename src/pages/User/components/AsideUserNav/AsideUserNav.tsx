  ...
  return (
    <div>
      <div className='my-4 flex items-center gap-4'>
        <div className='flex h-[50px] w-[50px] items-center justify-center rounded-full border-2 border-solid border-black/10'>
          {profile?.avatar ? (
            <img src={getUrlAvatar(profile.avatar)} alt='' className='h-full w-full rounded-full object-cover' />
          ) : (
            <svg
        ...
        <div>
          <p className='truncate text-sm font-semibold text-[#333]'>manhdungakp</p>
          <NavLink to={path.profile} className='mt-1 flex items-center gap-1 text-sm text-[#888]'>
            <svg
              width={12}
              height={12}
              viewBox='0 0 12 12'
      ...
      <div className='mt-10 flex flex-col gap-6'>
        <NavLink
          to={path.profile}
          className={({ isActive }) =>
            `flex items-center gap-3 text-sm hover:text-[#ee4d2d] ${isActive ? 'text-[#ee4d2d]' : 'text-[#888]'}`
          }
        >
